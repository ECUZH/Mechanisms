...
public class testCAXOR {
	...
	public void testCAXOR3() throws Exception {
		...
		items33.add(3);
		items33.add(4);
		List<Integer> items34 = new LinkedList<Integer>();
		items34.add(1);
		items34.add(2);
		List<Integer> items35 = new LinkedList<Integer>();
		items35.add(3);
		...
		Type bid3 = new CombinatorialType();
		bid3.addAtomicBid(atom31);
		bid3.addAtomicBid(atom32);
		bid3.addAtomicBid(atom33);
		bid3.addAtomicBid(atom34);
		bid3.addAtomicBid(atom35);
...
/*	@Test
	...
	public void testQuadraticPayment() {
		...
		CombinatorialType bid1 = new CombinatorialType();
		bid1.addAtomicBid(atom11);
		bid1.addAtomicBid(atom12);
		...
		CombinatorialType bid4 = new CombinatorialType();
		bid4.addAtomicBid(atom41);
		...
		//int[] allocatedAtoms = new int[ca.getWinnerIds().length];
		//for(int i = 0; i < ca.getWinnerIds().length; ++i)
		//	allocatedAtoms[i] = ca.getAllocatedBundle( ca.getWinnerIds()[i] );

		//List<Integer> bc = new LinkedList<Integer>();
		//ca.computeSEP(ca.getWinnerIds(), allocatedAtoms, vcg, bc);
		
		//assertTrue(ca.getPayments()[0] == 0.03);
		//assertTrue(ca.getPayments()[1] == 0.07);
		
		List<Double> payments = ca.computeCorePayments();
		assertTrue( payments.size() == 2);
		assertTrue( Math.abs( payments.get(0) - 0.00999999999 ) < 1e-6 ); 
		assertTrue( Math.abs( payments.get(1) - 0.09000000000 ) < 1e-6 );
	}
