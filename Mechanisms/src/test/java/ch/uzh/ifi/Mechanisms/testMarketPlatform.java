...
public class testMarketPlatform {
	...
	public void testMarketDemandDynamicBenchmark() throws Exception 
	{
		...
		for(int j = 0; j < numberOfSamples; ++j)
		{
			...
			List<Double> alloc2 = Arrays.asList(0., 1.);
			LinearThresholdValueFunction v12 = new LinearThresholdValueFunction(1, 1, alloc2);
			...
			surplus[j] = totalUtility;
		}	
		
		double expectedSurplus = 0.;
		for(int i = 0; i < numberOfSamples; ++i)
			expectedSurplus += surplus[i];
		System.out.println("Expected surplus: " + expectedSurplus / 100);
/*		int detAlloc = 2;																// Allocate only the 2nd DB with Pr[alloc]=1
		List<Double> marketDemand = mp.computeMarketDemand(0., detAlloc);
