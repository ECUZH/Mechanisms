...
public class ExpVCGPayments implements PaymentRule
{
	 ...
	 */
	public ExpVCGPayments(Allocation allocation, int numberOfBuyers, int numberOfItems, List<Type> bids, List<Double> costs, JointProbabilityMass jpmf, IloCplex cplexSolver)
	{
		_allocation = (AllocationEC)allocation;
	...
	public List<Double> computePayments() throws Exception 
	{
		...
		for(int i = 0; i < numberOfAllocatedBidders; ++i)
		{
			...
			if( isLLG() ) 
				auction.setPaymentRule("Exp-VCG_LLG");				//Faster allocation without using CPLEX.
			else
			{
				auction.setPaymentRule("Exp-VCG");
